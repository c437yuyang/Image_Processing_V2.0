	...
	for (int k = N; k != 0; --k) //行
	{
		for (int i = 0; i != w; ++i)//列
		{
			for (int j = 0; j != 3; ++j)
			{
				...
				{
					dst.m_pBits[j][N - k][i] = nFillColor;//上边缘
					dst.m_pBits[j][h - k][i] = nFillColor;//下边缘
				}
				else //nFillMode 在复制边缘像素的时候设为-1
				{
					if (i >= N && i <= w - 1 - N)
					{
						dst.m_pBits[j][N - k][i] = m_pBits[j][0][i - N];//上边缘
						dst.m_pBits[j][h - k][i] = m_pBits[j][h - 2 * N - 1][i - N];//下边缘
					}
					else if (i < N)
					{
						dst.m_pBits[j][N - k][i] = m_pBits[j][0][0];//上边缘
						dst.m_pBits[j][h - k][i] = m_pBits[j][h - 2 * N - 1][0];//下边缘
					}
					else if (i > w - 1 - N)
					{
						dst.m_pBits[j][N - k][i] = m_pBits[j][0][w - 2 * N - 1];//上边缘
						dst.m_pBits[j][h - k][i] = m_pBits[j][h - 2 * N - 1][w - 2 * N - 1];//下边缘
					}
					else {
	...
	for (int k = 0; k != N; ++k) //列
	{
		for (int i = N; i != h - N; ++i) //行
		{
			for (int j = 0; j != 3; ++j)
			{
				if (nFillColor == 0 || nFillColor == 255)
				{
					dst.m_pBits[j][i][k] = nFillColor;//左边缘
					dst.m_pBits[j][i][w - k - 1] = nFillColor;//右边缘
				}
				else //nFillMode 在复制边缘像素的时候设为-1
				{
					dst.m_pBits[j][i][k] = m_pBits[j][i - N][0]; //左边缘填充设为复制第一列像素
					dst.m_pBits[j][i][w - k - 1] = m_pBits[j][i - N][w - 2 * N - 1];  //底边缘填充设为复制最后一列像素
				}
			}
		}
	}
