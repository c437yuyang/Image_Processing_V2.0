			...
			for (int j = 0; j != 3; ++j)
			{
				...
				else //nFillMode 在复制边缘像素的时候设为-1
				{
					if (i >= N && i <= w - 1 - N)
					{
						...
						//dst.at(j, N - k, i) = m_pBits[j][0][i - N];//上边缘
						//dst.at(j, h - k, i) = m_pBits[j][h - 2 * N - 1][i - N];//下边缘
					}
					else if (i < N)
					{
						...
						//dst.at(j, N - k, i) = m_pBits[j][0][0];//上边缘
						//dst.at(j, h - k, i) = m_pBits[j][h - 2 * N - 1][0];//下边缘
					}
					else if (i > w - 1 - N)
					{
						...
						//dst.at(j, N - k, i) = m_pBits[j][0][w - 2 * N - 1];//上边缘
						//dst.at(j, h - k, i) = m_pBits[j][h - 2 * N - 1][w - 2 * N - 1];//下边缘
					}
				}
